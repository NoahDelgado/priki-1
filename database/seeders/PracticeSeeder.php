...
class PracticeSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ([
            ...
            ["Les tess", "ts n'ont pas besoin d'être testés. L'infrastructure, les frameworks et les bibliothèques pour les tests ont besoin de tests. Ne testez pas le navigateur ou les bibliothèques externes, sauf si vous en avez vraiment besoin. Testez le code que vous écrivez, pas celui des autres."],
            ["La troi", "sième fois que vous écrivez le même morceau de code est le bon moment pour l'extraire dans une aide générale (et écrire des tests pour elle). Les fonctions d'aide à l'intérieur d'un test n'ont pas besoin d'être testées ; lorsque vous les décomposez et les réutilisez, elles ont besoin de tests. La troisième fois que vous écrivez un code similaire, vous avez généralement une idée claire de la forme du problème général que vous résolvez."],
            ...
            ["Inévita", "blement, les commentaires de code deviennent des mensonges avec le temps. En pratique, peu de gens mettent à jour les commentaires lorsque les choses changent. Efforcez-vous de rendre votre code lisible et auto-documenté par de bonnes pratiques de nommage et un style de programmation connu."],
            ...
            ["La logi", "que est facile à tester en unité si elle est sans état et sans effet secondaire. Décomposez la logique en fonctions distinctes, plutôt que de la mélanger à du code sans état et rempli d'effets secondaires. La séparation du code avec état et du code avec effets secondaires en fonctions plus petites les rend plus faciles à simuler et à tester sans effets secondaires. (Moins de frais généraux pour les tests signifie des tests plus rapides.) Les effets secondaires doivent être testés, mais les tester une fois et les simuler partout ailleurs est généralement un bon modèle."],
            ["Les glo", "baux sont mauvais. Les fonctions sont meilleures que les types. Les objets sont probablement meilleurs que les structures de données complexes."],
            ["L'utili", "sation des types intégrés de Python - et de leurs méthodes - sera plus rapide que l'écriture de vos propres types (sauf si vous écrivez en C). Si les performances sont un facteur important, essayez de trouver un moyen d'utiliser les types intégrés standard plutôt que des objets personnalisés."],
            ["L'injec", "tion de dépendances est un modèle de codage utile pour être clair sur ce que sont vos dépendances et d'où elles viennent. (Faites en sorte que les objets, les méthodes, etc. reçoivent leurs dépendances en tant que paramètres plutôt que d'instancier eux-mêmes de nouveaux objets). Cela rend les signatures d'API plus complexes, c'est donc un compromis. Si vous vous retrouvez avec une méthode qui nécessite 10 paramètres pour toutes ses dépendances, c'est un bon signe que votre code en fait trop, de toute façon. L'article définitif sur l'injection de dépendances est \"Inversion of Control Containers and the Dependency Injection Pattern\", par Martin Fowler."],
            ["Plus vo", "us devez faire des simulations pour tester votre code, plus votre code est mauvais. Plus vous devez instancier et mettre en place du code pour pouvoir tester un comportement spécifique, plus votre code est mauvais. L'objectif est de créer de petites unités testables, ainsi que des tests d'intégration et fonctionnels de plus haut niveau pour vérifier que les unités coopèrent correctement."],
